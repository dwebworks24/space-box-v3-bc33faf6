...
const promises = [
  { icon: Shield, title: "Quality Assurance", desc: "Carefully selected materials, experienced workmanship, and site-level supervision ensure consistent quality." },
  { icon: Clock, title: "On-Time Delivery", desc: "Defined project timelines and milestone tracking help us complete projects as committed." },
  { icon: Gem, title: "Transparent Pricing", desc: "Clear scope of work, structured costing, and detailed quotations with no hidden charges." },
  { icon: Handshake, title: "Client-First Approach", desc: "Every design is developed around your lifestyle, operational needs, and long-term usability." },
  { icon: Leaf, title: "Sustainable Design", desc: "Thoughtful material selection and energy-efficient planning integrated into every project." },
  { icon: HeartHandshake, title: "Post-Project Support", desc: "We remain available for assistance and support even after project handover." },
];
...
const PromisesSection = () => {
  ...
  const gridY = useTransform(scrollYProgress, [0, 1], [80, -40]);
  const lineScale = useTransform(scrollYProgress, [0, 0.3], [0, 1]);

  return (
    <section ref={sectionRef} className="py-28 bg-background border-t border-border overflow-hidden">
      <div className="container mx-auto px-6 sm:px-10 md:px-14 lg:px-20">
        {/* Header with parallax */}
        <motion.div className="mb-20" style={{ y: headerY }}>
          <motion.div
            ...
            {promises.map((p) => (
              ...
              >
                <div className="flex items-start gap-5">
                  ...
                  >
                    <p.icon className="w-5 h-5 text-secondary transition-colors duration-500" strokeWidth={1.5} />
                  </motion.div>
                  <div>
